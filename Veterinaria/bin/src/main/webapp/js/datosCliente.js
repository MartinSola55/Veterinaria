	...
	$.ajax({
		...
		success : function(data) {
	        ...
	        for (let i = 0; i < data.length; i++) {
	            ...
	            tarjeta += "<button class='btn btn-outline-dark' onclick='modalEdit(" + data[i]["id"] + ")' >Ver</button>"
	            tarjeta += "<button class='btn btn-danger' onclick='modalDelete(" + data[i]["id"] + ")' >Eliminar</button>"
	            tarjeta += "</div>"
	            tarjeta += "</div>"
	            tarjeta += "</div>"
	            tarjeta += "<br/>"
